...
| 항목 | 상태 | 조치 |
|---|---|---|
| **개인정보처리방침 URL (공개 호스팅)** | 🟡 페이지 생성됨 (`apps/web/src/app/privacy/page.tsx` + `/terms`), **배포·연락처 입력 대기** | 2026-06-14 `/privacy`·`/terms` 라우트 추가(빌드 통과). justdo.co.kr 재배포 + 페이지 내 `CONTACT_EMAIL`·시행일 실제값 교체 필요. |
...
- [🟡] `/privacy` (+`/terms`) 페이지 생성 — justdo.co.kr **재배포** + `CONTACT_EMAIL`/시행일 실제값 교체 대기
