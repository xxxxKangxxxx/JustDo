...
| 목표 | 설정 내부 목표 관리 화면. 연간/월간 목표 추가, 수정, 고정, 삭제 |
| 설정 | 홈 우측 상단 아이콘으로 진입. 계정, 알림, 카테고리/태그, 디스플레이, 구독, 데이터, 앱 정보 |
...
|------|------|
| 폼팩터 | 모바일 단일 컬럼 |
| 네비게이션 | 하단 바 유지. 단기적으로 `홈` 단일 탭을 중앙에 배치하고 설정은 홈 우측 상단 아이콘으로 이동. 후속 `함께` 기능 도입 시 `홈 / 함께`로 확장 |
